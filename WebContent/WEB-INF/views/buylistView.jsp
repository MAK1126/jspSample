...
<script>
	...
	const prizeText = {
			'1등': { message: '1등 입니다!', money: 1680000000 },
		  	...
		  	'3등': { message: '3등 입니다!', money: 1000000 },
		  	'4등': { message: '4등 입니다!', money: 100000 },
		 	'5등': { message: '5등 입니다!', money: 50000 },
		 	'6등': { message: '6등 입니다!', money: 5000 },
		 	'7등': { message: '7등 입니다!', money: 1000 },
	...
	function prizeCheck(obj){
		...
		const buyNum = obj.parentElement.lastElementChild.value;
		const receipt = obj.parentElement.firstElementChild.nextElementSibling.value;
		const code = obj.parentElement.firstElementChild.nextElementSibling.nextElementSibling.value;
		...
		$.ajax({
			...
			switch (data) {
				case "1등":
					Swal.fire('당첨', '1등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
						return Promise.all([
			    		$.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":1680000000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
					break;
			    case "2등":
			    	Swal.fire('당첨', '2등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
			    		 return Promise.all([
			    		 $.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":120000000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
			     	break;
			    case "3등":
			    	Swal.fire('당첨', '3등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
			    		 return Promise.all([
			    		 $.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":1000000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
			    	break;
			    case "4등":
			    	Swal.fire('당첨', '4등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
			    		 return Promise.all([
			    		 $.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":100000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
			    	break;
			    case "5등":
			    	Swal.fire('당첨', '5등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
			    		 return Promise.all([
			    		 $.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":50000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
			    	break;
			    case "6등":
			    	Swal.fire('당첨', '6등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
			    		 return Promise.all([
			    		 $.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":5000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
			    	break;
			    case "7등":
			    	Swal.fire('당첨', '7등 입니다! <br> 확인 버튼을 누르면 당첨금이 지급됩니다', 'success').then(function() {
			    		 return Promise.all([
			    		 $.ajax({url:"/Team4/prize",type:"post",dataType:"text",data:{"money":1000,"code":code}}).then(function(){
			    			 window.location.reload();
			    		 })])
					})
			    	break;
			    case "꽝":
			    	Swal.fire('꽝', '꽝입니다..<br>다음기회에', 'warning');
			    break;
			  }
...
<body>
	<% 	ArrayList<lottery> buyList = (ArrayList<lottery>)request.getAttribute("buyList");
	...
	<div>
		...
		<thead>
			...
			<% for(lottery buy : buyList){
                ...
                %>
			<form name="buyList" action="./buylist" method="post">
				<tbody>
				<tr>
					<td><%=type %></td>
					<td><%=buy.getWeek() %></td>
					<td><% String[] numbers = buy.getNumber().split(",");
						out.print(numbers[0] + "조 ");
